...
namespace BizHawk.Client.EmuHawk
{
    public partial class MainForm : Form
    {
        ...
        public int ProgramRunLoop()
        {
            ...
            // place folders to maintain a variable for lua console and then load lua script on second iteration of loop
            LuaConsole current = new LuaConsole();
            int second = 0;
            ...
            for (;;)
            {
                ...
                if (first)
                {
                    LoadRomFromRecent("*OpenRom*Super Mario Bros. (Japan, USA).nes");
                    current = GlobalWin.Tools.Load<LuaConsole>();

                    //GlobalWin.Tools.LuaConsole.UpdateDialog();
                ...
                //string fullName = "C:\\Source\\SpartaHacks17\\Super Mario Bros. (Japan, USA).nes";
                //var lra = new LoadRomArgs { OpenAdvanced = new OpenAdvanced_OpenRom { Path = fullName } };
                //LoadRom(fullName, lra);
            }
        ...
        private void StepRunLoop_Core(bool force = false)
        {
            ...
            if(MakeRewind==true)
            {
                ...
                if(y>100)
                {
                    GlobalWin.MainForm.SaveState("auto.state","auto.state", true);
                    MakeRewind = false;
                    x = 0;
                    y = 0;
                }
